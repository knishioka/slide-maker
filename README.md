...
```bash
# 1. インタラクティブなタスク選択とプロジェクト進捗確認
./scripts/check-progress.sh

# スクリプトが以下を表示します：
# - プロジェクト全体の進捗統計
# - アクティブタスク・ブロッカー・最近完了したタスク
# - 📋 インタラクティブメニュー：
#   1. 新しいタスクを開始（対話式選択）
#   2. レポートのみ表示
#   3. 既存worktreeの管理
...
# 2. タスク選択プロセス：
#   - 利用可能なTODOタスクが優先度・工数と共に表示
#   - 詳細情報表示オプション（依存関係・説明など）
#   - タスク選択後、自動でworktree作成とタスク開始処理
...
- **コンフリクト回避**: ブランチ間での競合を最小化
- **明確な責任範囲**: タスクごとに専用の作業領域
